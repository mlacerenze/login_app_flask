...
def log_in_user():
  if current_user.is_authenticated:
    flash('You are already register!')
    return redirect(url_for('authentication.homepage'))
  form = LoginForm()
  if form.validate_on_submit():
...
def scrapy_data():
  ...
  if form.validate_on_submit():
    ...
    # extract link in browser (inspect). Search in element tab
    # //ol[@class='ui-search-layout ui-search-layout--stack']//div[@class='ui-search-item__group ui-search-item__group--title']//a[@href]
    ...
    if len(data_articles) == 0:
      data = {'links': ['Information not found']}
    else:
      data = {'links': data_articles}
    return render_template('scrapy_data.html', **data) # this generate links list
